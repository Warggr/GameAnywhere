...
class Poker(Game):
    ...
    def play_game(self) -> GameSummary:
        ...
        try:
            ...
            self.revealed_cards.extend(self.deck.draw() for _ in range(3)) # TODO: OPTIM: add a draw_multiple function
            self.betting_round()
        ...
        for i, player in enumerate(self.players):
            if not player.folded:
                best_hand_for_player = max(PokerHand(cards) for cards in combinations(self.revealed_cards + player.hand, 5))
        ...
        for turn_counter in cycle(range(len(self.agents))):
            ...
            if decision == 'fold':
                ...
                if sum(not player.folded for player in self.players) == 1:
                    ...
                    last_remaining_player = next(filter(lambda args: not player[1].folded, enumerate(self.players)))[0]
...
hand_value_names_ranked = [ 'high_card', 'pair', 'two_pairs', 'three_of_a_kind', 'straight', 'flush', 'full_house', 'four_of_a_kind', 'straight_flush' ]

class PokerHand:
    def __init__(self, cards : List[PokerCard]):
        self.cards = list(cards)
        self.cards.sort(key=lambda card: card.value) # already sorting by value because it will be useful for most algorithms
        self.values = self.cards_by_value()

        for i, hand_value_name in sorted( enumerate(hand_value_names_ranked) , reverse=True):
            self.hand_subvalue = hand_values_by_name[hand_value_name](self)
            if self.hand_subvalue is not None:
                self.hand_value = i
                break
        assert self.hand_subvalue is not None # at least one hand type (High Card) should have matched

    def __lt__(self, other : 'PokerHand') -> bool:
        if not isinstance(other, type(self)):
            raise ValueError(f"object of type PokerHand can't be compared with object of other type {type(other)}")
        return (self.hand_value, self.hand_subvalue) < (other.hand_value, other.hand_subvalue) # tuple comparison is so convenient
    ...
    def is_straight(self) -> PokerCard.Value | None:
        if self.cards[4].value == int(self.cards[0].value) + 4:
            return self.cards[4]
        elif self.cards[0] == PokerCard.Value.ACE and self.cards[1] == 10 and self.cards[4] == PokerCard.Value.KING:
    ...
    @hand_value
    def is_three_of_a_kind(self) -> Tuple[PokerCard.Value, PokerCard.Value, PokerCard.Value] | None:
    ...
    @hand_value
    def is_two_pairs(self) -> Tuple[PokerCard.Value, PokerCard.Value, PokerCard.Value] | None:
    ...
    @hand_value
    def is_pair(self) -> Tuple[PokerCard.Value, PokerCard.Value, PokerCard.Value, PokerCard.Value] | None:
    ...
    def cards_by_value(self) -> List[Tuple[int, PokerCard.Value]]:
        ...
        return sorted((number, value) for value, number in cards_by_value.items()) # sorted first by number of cards, and then by card value
