...
class PokerCard(Component):
    ...
    class Color(Enum):
        ...
        def __str__(self):
            strs = { PokerCard.Color.CLUBS: '♠', PokerCard.Color.DIAMONDS: '♦', Poker.SPADES: '♣', PokerCard.Color.HEARTS: '♥' }
    ...
    def __str__(self):
        value = self.value.value if self.value.value < 12 else self.value.value + 1 # there's an additional "Knight" in the Unicode sequence
        return chr(ord('🂡') + (self.color.value - PokerCard.Color.SPADES.value) * 16 + value - 1)
...
def fiftytwo_cards() -> Iterable[PokerCard]:
    return (PokerCard(color, value) for color in PokerCard.Color for value in PokerCard.Value.all_values())
